...
import CreateBetModal from '@/components/Sport/CreateBetModal';
...
const SportItem: React.FC<Props> = (props) => {
  ...
  return scheduleData.scheduleId.toString() !== '0' ? (
    <Card>
      ...
      <CardFooter className="gap-3 flex justify-between">
        ...
        <div className="flex gap-1">
          <Button
            color={!scheduleData.status ? 'default' : 'primary'}
            size="sm"
            onClick={() => onClick(scheduleData.scheduleId)}
            disabled={!scheduleData.status}
          >
            Bet
          </Button>
        </div>
